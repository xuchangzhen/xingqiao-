...
- macOS、Windows 桌面端将点“接收”的文件流式写入本机临时收件箱，并以真实本地文件提供原生拖拽；可多选、预览、只保存选中的项目。macOS 收件箱采用 Finder 式缩略图网格，菜单栏浮窗可直接接收等待项，也可通过系统文件选择器把文件加入发送区；Windows 从悬浮收件箱拖入聊天。
- macOS、Windows 与 Android App 都会显示当前版本并可检查 GitHub Release 更新。桌面端将新版下载到“下载”文件夹，退出 App 后由用户解压并替换旧版；Android 下载 APK 后交由系统安装确认。三端使用同一套星桥图标。
...
默认构建会查询星桥公开仓库的“最新 Release API”。如果要改用自己的公开仓库，可在签名 Release 构建时覆盖更新地址：
...
接收时，文件会先流式写入 `~/Library/Caches/Xingqiao/Inbox`。在内嵌网页的“按住这里拖入聊天”卡片或“星桥临时收件箱”悬浮窗中拖动条目，都会向目标应用交付真正的 `file://` 文件，因此可用于 ChatGPT、Codex 和其他支持 macOS 文件投放的应用。收件箱会以 Finder 式网格显示真实缩略图、选择圆点、名称、大小和时间；可用 ⌘/⇧ 多选或“全选”，双击或点“预览”查看，且“保存选中项到…”只会移走当前选中的文件。浮窗的“接收等待项”会接收第一个等待传输而不打开主网页；“发送文件”会打开系统选择器，并把所选文件以按需读取的方式加入网页发送区。直接退出星桥会删除尚未保存的临时文件。关闭主窗口只会隐藏到菜单栏，可通过菜单栏的“显示星桥”重新打开。
