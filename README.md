...
- Chrome / Edge 的接收目录窗口会从“桌面”打开；系统目录受浏览器保护不能写入，请选择桌面或其他普通文件夹。若 macOS 拒绝桌面访问，请在“系统设置 → 隐私与安全性 → 文件与文件夹”中允许 Chrome 访问桌面。
...
- 文件传输采用有界的接收端落盘确认窗口（最高 1 MB 在途数据）；Android Release 使用 ArrayBuffer 原生写入，避免 Base64 转换拖慢局域网传输，发送与接收界面都会显示进度、速度和当前文件。
...
局域网直连时，文件不会经过运营商网络；速度由 Wi-Fi/网口、路由器和设备性能决定。页面会显示“局域网直连传输”。若显示“公网中转”，说明直连未建立，速度会受两端公网带宽和中转服务器影响。
